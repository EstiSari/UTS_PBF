export default (state, action) => {
    ...
    switch (action.type) {
      case "ADD_CART":
        cartIndex = carts.findIndex(cart => cart._id === action.payload);
        if (cartIndex !== -1) {
          ...
          return {
            ...state,
            carts: [...state.carts, { _id: action.payload, quantity: 1 }]
          };
        }
      ...
      case "UPDATE_CART":
        cartIndex = carts.findIndex(cart => cart._id === action.payload._id);
        if (cartIndex !== -1) {
